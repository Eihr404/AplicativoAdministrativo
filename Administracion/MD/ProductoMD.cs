...
namespace Administracion.MD
{
    internal class ProductoMD
    {
        ...
        public List<string> ObtenerCategoriasMD()
        {
            ...
            while (dr.Read())
            {
                string item = $"{dr.GetString(0)}|{dr.GetString(1)}";
                categorias.Add(item);
            }
        ...
        /* Obetener todas las clasificaciones */
        public List<string> ObtenerClasificacionesMD()
        {
            List<string> clasificaciones = new();

            string sql = @"
                SELECT CLA_Codigo, CLA_Nombre
                FROM CLASIFICACION
                ORDER BY CLA_Nombre";
        ...
        public List<string> ObtenerUnidadesMedidaMD()
        {
            List<string> unidades = new();

            string sql = @"
                SELECT UME_Codigo, UME_Descripcion
                FROM UNIDAD_MEDIDA
                ORDER BY UME_Descripcion";
